...
`firmware/src/wifi_manager.h/.cpp` stores WiFi credentials and the local pairing token in NVS (via `Preferences`) and owns the station connection lifecycle. WiFi stays off until credentials are provisioned over USB, so a USB-only setup never pays for an idle radio. Once connected, it registers a MAC-suffixed `<hostname>.local` mDNS name (`ESPmDNS`) so the web app, the QR code, and the on-device screen can all point at a stable name instead of a numeric IP that can change on a DHCP renewal - the raw IP stays available everywhere as a fallback for networks where `.local` doesn't resolve.
