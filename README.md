...
4. Once connected, the same page shows an **Open Cyberclip** link (when [`kAppBaseUrl`](firmware/matrix_display.ino) is configured, see below) - tap it once your phone has switched back to its normal WiFi or mobile data (usually just a few seconds, since phones automatically rejoin a known network once the temporary setup network disappears). It opens the Cyberclip web app already pointed at this board, ready to connect with one tap - no typing, no camera needed.
5. The board's own screen shows the same information for about 20 seconds after connecting, in case that's easier: a **QR code** encoding the same link (scan it after switching your phone off the setup network), or, if `kAppBaseUrl` isn't configured, the board's `<name>.local` address as plain text (with its numeric IP underneath, in case `.local` doesn't resolve on your network) to enter manually under **Connect over WiFi**.
